-- incremental_etl.sql -- Для secondary load

-- 1. Очистка и загрузка новых сырых данных
...
-- 2. Вставка НОВЫХ customers (которых ещё нет в dim)
...
-- ================================================================
-- 3. ИСПРАВЛЕННАЯ ЛОГИКА SCD
-- ================================================================

-- 3.1. SCD Type 1: Обновляем только имя (customer_name), если оно изменилось
-- Мы НЕ трогаем segment здесь, чтобы не испортить историю.
...
-- 3.2. SCD Type 2: Обработка изменения Segment (история)
WITH changed_customers AS (
    -- 1. Находим, кто изменил сегмент
    SELECT DISTINCT 
        s.customer_id,
    ...
    WHERE c.segment <> s.segment -- Сравниваем текущий Stage со старым Core
    GROUP BY s.customer_id
),
closed_old_versions AS (
    -- 2. Закрываем старую версию (ставим valid_to и is_current = false)
    UPDATE core.customers_dim c
    SET 
        valid_to = cc.change_date - INTERVAL '1 day', -- или просто cc.change_date, зависит от вашей логики >=/<
        is_current = FALSE
    FROM changed_customers cc
    WHERE c.customer_id = cc.customer_id 
      AND c.is_current = TRUE
    RETURNING c.customer_id -- возвращаем ID, чтобы знать, кого закрыли
),
new_versions_to_insert AS (
    -- 3. Готовим данные для новой версии из Stage
    SELECT DISTINCT ON (s.customer_id)
        s.customer_id,
        s.customer_name,
    ...
    -- Берем записи, которые случились в дату изменения или позже
    WHERE TO_DATE(s.order_date, 'MM/DD/YYYY') >= cc.change_date
    ORDER BY s.customer_id, TO_DATE(s.order_date, 'MM/DD/YYYY') ASC 
)
-- 4. Вставляем новые активные версии
INSERT INTO core.customers_dim (customer_id, customer_name, segment, valid_from, valid_to, is_current)
SELECT * FROM new_versions_to_insert
ON CONFLICT (customer_id, valid_from) DO NOTHING;

-- 5. Geography SCD Type 2 (рекомендую добавить поля в таблицу: valid_from DATE, valid_to DATE, is_current BOOLEAN, UNIQUE(country,region,state,city,valid_from))
-- Если полей нет — сначала ALTER TABLE добавить их + обновить существующие записи (valid_from = '2014-01-01', valid_to=NULL, is_current=TRUE)

-- Закрытие старых + вставка новых версий geography аналогично customers (опционально, если требуется история перемещений)
...
-- 6. Вставка новых фактов (дубликаты пропускаются)
-- 6. Вставка новых фактов (Полный код без сокращений)
...
SELECT
    ...
    s.discount,
    s.profit,
    -- Подзапрос для поиска ключа клиента (берем текущую активную версию)
    (SELECT customer_pk 
     FROM core.customers_dim c 
     WHERE c.customer_id = s.customer_id 
       AND c.is_current = TRUE),
       
    -- Подзапрос для поиска ключа продукта
    (SELECT product_pk 
     FROM core.products_dim p 
     WHERE p.product_id = s.product_id),
     
    -- Подзапрос для поиска ключа географии
    (SELECT geo_pk 
     ...
     WHERE g.country = s.country 
       ...
       AND g.state = s.state 
       AND g.city = s.city
     LIMIT 1) -- LIMIT 1 на случай, если вдруг есть дубли в гео, чтобы запрос не упал
FROM
    stage.superstore_raw s
ON CONFLICT (order_id, row_id) DO NOTHING;

-- 7. Refresh mart (полная перезапись или incremental)
-- 7. Refresh mart (полная перезапись)
...
JOIN core.customers_dim c ON f.customer_fk = c.customer_pk 
    -- Соединяем так, чтобы заказ попадал в правильный исторический период клиента
